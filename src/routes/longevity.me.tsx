...
export const Route = createFileRoute("/longevity/me")({
  head: () => ({ meta: [{ title: "My Rebél at 50+ — Member" }] }),
...
function MyLongevityPage() {
    ...
    return (
      <SiteShell>
        <section className="container mx-auto max-w-lg px-5 py-20 text-center">
          ...
          <h1 className="mt-4 text-2xl md:text-3xl font-black">You&rsquo;re not enrolled in Rebél at 50+ yet</h1>
  ...
  const downloadProgressPdf = () => {
    ...
    doc.setFontSize(20);
    doc.text("Rebél at 50+ — Progress Timeline", margin, y);
    y += 24;
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Rebél  ·  Page ${i} of ${pageCount}`, margin, pageH - 20);
    }

    doc.save(`Rebél-50plus-progress-${member.first_name}-${new Date().toISOString().slice(0, 10)}.pdf`);
  };
    ...
    <SiteShell>
      <section className="container mx-auto px-5 py-8 md:py-12 max-w-3xl">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 md:p-7">
          <p className="text-xs uppercase tracking-[0.18em] text-primary">Rebél at 50+</p>
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-5 md:p-6">
          ...
          {familyUrl && (
            <div className="mt-4 rounded-xl bg-muted/30 border border-border p-3 flex items-center gap-2">
              ...
              {member.family_phone && (
                <a
                  href={`https://wa.me/${member.family_phone.replace(/\D/g, "")}?text=${encodeURIComponent(`Hi! Here's my Rebél progress: ${familyUrl}`)}`}
                  target="_blank"
                  rel="noreferrer"
